...
from .gemini_client import EvalGeminiProvider
...
def run_trial(probe, provider: EvalGeminiProvider, repeat: int) -> Trial:
...
def run_eval(
    ...
    provider = EvalGeminiProvider(model=model, temperature=temperature, use_cache=use_cache)
    trials: list[Trial] = []
    for p in probes:
...
def main() -> None:
    ...
    ap.add_argument(
        "--outdir",
        default=str(Path(__file__).resolve().parents[1] / "eval_runs"),
