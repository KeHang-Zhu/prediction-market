...
REPLAY_MAX_GAP = 8.0
# the project root; scenario *.yaml configs the UI can switch between live in scenarios/
...
# create_market enabled. Other *.yaml in scenarios/ (e.g. scenarios/archive/) are
# intentionally NOT offered in the picker.
SCENARIO_FILES = ("demo.yaml", "demo5.yaml", "llm5_only.yaml", "llm5_open.yaml", "llm5_orders.yaml")

# user-built scenario templates (from the web builder) live in scenarios/templates/, listed
# alongside the built-ins in the picker as "templates/<slug>.yaml".
_TEMPLATES_DIR = _SCENARIOS_DIR / "templates"
...
class SimulationSession(Session):
    ...
    def scenarios(self) -> list[dict]:
        ...
        out = [{"file": f, "builtin": True}
               for f in SCENARIO_FILES if (_SCENARIOS_DIR / f).exists()]
    ...
    def load_config_file(self, name: str) -> bool:
        """Switch to a built-in scenario (whitelisted, scenarios/) or a user template
        (scenarios/templates/*.yaml, path-confined)."""
        if name in SCENARIO_FILES and (_SCENARIOS_DIR / name).exists():
            path = _SCENARIOS_DIR / name
        elif name.startswith("templates/") and name.endswith(".yaml"):
            path = _TEMPLATES_DIR / name[len("templates/"):]
